...
namespace Elearning.UI.Components.Pages.NghiepVu.HoSoGiaoVien
{
    public partial class Index : ComponentBase
    {
        ...
        private ColumnKeyGridSort<HoSoGiaoVienDto> _roleTieuDeSort = new("TenGiaoVien");

        protected FluentDataGrid<HoSoGiaoVienDto> Grid { get; set; } = default!;
        ...
        protected async Task OpenAddModal()
        {
            ...
            await DialogService.ShowDialogAsync<Edit>(parameters, new DialogParameters { Title = "Thêm mới hồ sơ", Width = "700px", Modal = true });
        }
        ...
        protected async Task EditAction(Guid Id)
        {
            ...
            await DialogService.ShowDialogAsync<Edit>(parameters, new DialogParameters { Title = "Chỉnh sửa hồ sơ", Width = "700px", Modal = true });
        }

        protected async Task OpenDetailsModal(Guid Id)
        {
            var parameters = new EditOrUpdateParametersDto { Id = Id };
            await DialogService.ShowDialogAsync<View>(parameters, new DialogParameters { Title = "Chi tiết hồ sơ", Width = "700px", Modal = true });
        }
        ...
        protected async Task OpenModalDelete(Guid id)
        {
            ...
            {
                var apiRequest = new ApiRequestModel { ApiService = ServicesRegistryEnum.ServicePortal, Endpoint = $"/HoSoGiaoVien/{id}", HasAuthorization = true };
        ...
        private async ValueTask<GridItemsProviderResult<HoSoGiaoVienDto>> LoadDatas(GridItemsProviderRequest<HoSoGiaoVienDto> request)
        {
            var apiRequest = new ApiRequestModel { ApiService = ServicesRegistryEnum.ServicePortal, Endpoint = "/HoSoGiaoVien/getpaged", HasAuthorization = true };
            var baseQuery = new BaseQuery
            {
                draw = 1,
                SearchIn = new List<string> { "TenGiaoVien" }, // Backend phải cấu hình search theo Tên User
            ...
            var result = await CallService.Post<DataTableJson<HoSoGiaoVienDto>>(apiRequest, baseQuery);
            ...
            return GridItemsProviderResult.From(new List<HoSoGiaoVienDto>(), 0);
        }
