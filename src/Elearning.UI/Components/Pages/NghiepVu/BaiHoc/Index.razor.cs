...
namespace Elearning.UI.Components.Pages.NghiepVu.BaiHoc
{
    public partial class Index : ComponentBase
    {
        ...
        private ColumnKeyGridSort<BaiHocDto> _roleTieuDeSort = new("TieuDe");

        protected FluentDataGrid<BaiHocDto> Grid { get; set; } = default!;
        ...
        protected async Task OpenAddModal()
        {
            ...
            await DialogService.ShowDialogAsync<Edit>(parameters, new DialogParameters { Title = "Thêm mới bài học", Width = "700px", Modal = true });
        }
        ...
        protected async Task EditAction(Guid Id)
        {
            ...
            await DialogService.ShowDialogAsync<Edit>(parameters, new DialogParameters { Title = "Chỉnh sửa bài học", Width = "700px", Modal = true });
        }

        protected async Task OpenDetailsModal(Guid Id)
        {
            var parameters = new EditOrUpdateParametersDto { Id = Id };
            await DialogService.ShowDialogAsync<View>(parameters, new DialogParameters { Title = "Chi tiết bài học", Width = "700px", Modal = true });
        }
        ...
        protected async Task OpenModalDelete(Guid id)
        {
            ...
            {
                var apiRequest = new ApiRequestModel { ApiService = ServicesRegistryEnum.ServicePortal, Endpoint = $"/BaiHoc/{id}", HasAuthorization = true };
        ...
        private async ValueTask<GridItemsProviderResult<BaiHocDto>> LoadDatas(GridItemsProviderRequest<BaiHocDto> request)
        {
            var apiRequest = new ApiRequestModel { ApiService = ServicesRegistryEnum.ServicePortal, Endpoint = "/BaiHoc/getpaged", HasAuthorization = true };
            var baseQuery = new BaseQuery
            {
                draw = 1,
                SearchIn = new List<string> { "TieuDe" },
            ...
            var result = await CallService.Post<DataTableJson<BaiHocDto>>(apiRequest, baseQuery);
            ...
            return GridItemsProviderResult.From(new List<BaiHocDto>(), 0);
        }
